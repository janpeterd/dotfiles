...
vec4 applyFakeHDR(vec4 color, float brightness, float contrast,
                  float saturation, float bloom) {
  ...
  color.rgb =
      mix(color.rgb, vec3(1.0), pow(max(0.0, luminance - 1.0 + bloom), 4.0));
  return color;
}
...
vec4 applyVignette(vec4 color) {
  ...
  float alpha = smoothstep(radius, radius - radius * softness, distance) *
                intensity; // calculate alpha value for vignette effect
  return mix(vec4(0.0, 0.0, 0.0, alpha), color,
             alpha); // mix black with color using calculated alpha value
}
...
vec4 applyPhosphorGlow(vec2 tc, vec4 color, sampler2D tex) {
  ...
  float factor = mix(mix(0.09, mix(0.005, 0.0075, (averageColor - 0.1) / 0.1),
                         step(0.01, averageColor)),
                     0.0005, step(0.02, averageColor));
  ...
  sum = pixels[0] + pixels[1] + pixels[2] + pixels[3] + pixels[4] + pixels[5] +
...
vec4 applyAdaptiveSharpen(vec2 tc, vec4 color, sampler2D tex) {
  ...
  float delta = (dot(color_no_alpha, vec3(0.333333)) +
                 dot(color_tl_no_alpha, vec3(0.333333)) +
                 dot(color_tr_no_alpha, vec3(0.333333)) +
                 dot(color_bl_no_alpha, vec3(0.333333)) +
                 dot(color_br_no_alpha, vec3(0.333333))) *
                    0.2 -
                dot(color_no_alpha, vec3(0.333333));
  vec3 sharp_color_no_alpha =
      color_no_alpha + min(vec3(0.0), vec3(delta * sharpness));
...
vec4 applyScanlines(vec2 tc, vec4 color) {
  float scanline = (cos(tc.y * SCANLINE_FREQUENCY + SCANLINE_TIME) *
                    sin(tc.y * SCANLINE_FREQUENCY + SCANLINE_TIME)) *
                   SCANLINE_THICKNESS;
...
vec4 applyChromaticAberration(vec2 uv, vec4 color) {
  ...
  vec2 offset =
      (uv - center) *
      CHROMATIC_ABERRATION_STRENGTH; // calculate the offset from the center
...
vec4 reduceGlare(vec4 color) {
  // Calculate the intensity of the color by taking the average of the RGB
  // components
  ...
  float glareIntensity =
      smoothstep(maxIntensity - 0.02, maxIntensity, intensity);
...
// - bloom: the intensity of the bloom effect. Should be a value between 0
// and 1.
vec4 applyFakeHDREffect(vec4 inputColor, float brightness, float contrast,
                        float saturation, float bloom) {
  ...
  if (brightness < minBrightness || brightness > maxBrightness) {
    return vec4(0.0, 0.0, 0.0,
                1.0); // Return black with alpha of 1.0 to indicate error
  }
...
vec4 bilateralFilter(sampler2D tex, vec2 uv, vec4 color, float sampleRadius,
                     float noiseThreshold, float intensity) {
  ...
  float weight = exp(-0.5 * (dist * dist + colorDist * colorDist * intensity) /
  ...
  weight = exp(-0.5 * (dist * dist + colorDist * colorDist * intensity) /
               (sampleRadius * sampleRadius));
  filteredColor += sample * weight;
  totalWeight += weight;
  ...
  return mix(color, filteredColor,
             step(noiseThreshold, length(filteredColor - color)));
}

vec4 supersample(sampler2D tex, vec2 uv, float sampleRadius,
                 float noiseThreshold, float intensity) {
  ...
  return bilateralFilter(tex, uv, color, sampleRadius, noiseThreshold,
                         intensity);
}
void main() {
  ...
  vec4 color =
      supersample(tex, tc, SAMPLING_RADIUS, NOISE_THRESHOLD, DENOISE_INTENSITY);
  ...
  color = mix(applyFakeHDREffect(color, HDR_BRIGHTNESS, HDR_CONTRAST,
                                 HDR_SATURATION, HDR_BLOOM),
              color, 0.5);
  ...
  color = applyBorder(tc_no_dist_normalized, color,
                      1.0 - BORDER_SIZE * BORDER_RADIUS, BORDER_COLOR);
