...
class MultiSelectList implements Component {
   ...
   public onSubmitWithComment?: (result: string[]) => void;
   ...
   private getSelectionResult(): string[] {
   ...
   handleInput(data: string): void {
      ...
      if (matchesKey(data, Key.tab)) {
         ...
         if (result.length > 0) this.onSubmitWithComment?.(result);
         return;
      }
...
class WrappedSingleSelectList implements Component {
   ...
   handleInput(data: string): void {
      ...
      if (matchesKey(data, Key.tab) && count > 0) {
         if (this.isCommentToggleRow(this.selectedIndex, filteredOptions)) {
            this.toggleComment();
            return;
         }
         ...
         if (result) this.onSubmitWithComment?.(result);
         return;
      }
...
class AskComponent extends Container {
   ...
   private updateHelpText(): void {
      ...
      if (this.allowMultiple) {
         const hints = [
            ...
            literalHint(theme, "tab", "add context"),
            commentHint,
            promptScrollHint,
            overlayHint,
   ...
   private ensureSingleSelectList(): WrappedSingleSelectList {
      ...
      list.onSubmitWithComment = (result) => this.beginSelectionComment([result]);
   ...
   private beginSelectionComment(selections: string[]): void {
   ...
   private handleSelectionSubmit(selections: string[], wantsComment: boolean): void {
      if (this.allowComment && wantsComment) {
         this.beginSelectionComment(selections);
         return;
      }
